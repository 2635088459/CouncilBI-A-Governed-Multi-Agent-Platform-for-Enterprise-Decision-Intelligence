...
| ID | 需求 | 状态 |
|---|---|---|
| FR-FV03-018 | 关键词打分必须使用 BM25(词频/逆文档频率/长度归一化),在每次请求的权限过滤候选集合上计算,而不是预先构建的全局索引。 | 已实现 |
| FR-FV03-019 | 关键词分词必须支持中文文本,不能只支持 ASCII 单词 token。 | 已实现 |
| FR-FV03-020 | 在应用 0.60/0.35 融合权重之前,BM25 分数必须归一化到与现有余弦相似度分量可比的区间。 | 已实现 |
